...
import { push, ref, set, getDatabase, get, child, orderByChild, equalTo, update } from "firebase/database";
 ...
 const  setusersubscribed = () => {
  ...
  const getusersubscribed = () => {
    ...
    get(data).then((snap) => {
      ...
      if (snap.val() != null) {
        console.log(snap.val()["subscribe"])
        // setData(Object.values(snap.val()))
        setsub(snap.val()["subscribe"])
              ...
              <nav className="list-none mb-10">
                {
                  Data.map((v, i) => {
                    return <li>
                      <a className="text-black hover:text-green-600" onClick={() => pagechg(v.category_name)}>
                        {v.category_name}
                      </a>
                    </li>
                  })
